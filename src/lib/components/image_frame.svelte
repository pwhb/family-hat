...
{#if variant === 'polaroid'}
   <figure
    class="group relative inline-block w-64 rounded-sm bg-stone-50 p-3 pb-5 shadow-xl ring-1 ring-black/5 dark:bg-stone-100 dark:text-stone-900 transition-transform duration-300 ease-out hover:scale-105 hover:z-10 {rotationClasses[rotation]}"
>
...
{:else if variant==='filmstrip'}
    ...
    <div class="group relative inline-block transition-transform duration-300 hover:scale-105 {rotationClasses[rotation]}">
        <div class="flex flex-col items-center bg-stone-950 p-2 text-stone-100 shadow-2xl rounded-sm ring-1 ring-white/10">
            ...
            <div class="relative my-1 aspect-4/3 w-64 overflow-hidden bg-black">
                <img
                    ...
                    loading="lazy"
                />
                <div class="pointer-events-none absolute inset-0 ring-1 ring-inset ring-black/40"></div>
            </div>

            <div class="flex w-full items-center justify-between px-2 py-1 text-[9px] font-mono text-amber-500/70">
                <span>KODAK 400</span>
                {#each Array(3) as _}
                    <div class="h-3 w-2 rounded-[1px] bg-stone-900 shadow-inner ring-1 ring-stone-800"></div>
                {/each}

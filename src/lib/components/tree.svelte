    ...
    >
        {#each svgPaths as path, idx}
            <path
                ...
                stroke-width={hoveredPathIdx === idx ? '4' : '1.5'}
            ...
            <path
                ...
                // UNIFIED POINTER TRACKING LAYER
                onpointerdown={(e) => {
                    // Instantly highlights path when tapped/clicked
                    handlePathIntersection(e.clientX, e.clientY);
                }}
                onpointermove={(e) => {
                    // Tracks drag intersections dynamically across screens
                    handlePathIntersection(e.clientX, e.clientY);
                }}
                onpointerleave={() => {
                    hoveredPathIdx = null;
